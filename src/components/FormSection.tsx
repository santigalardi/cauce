...
export default function FormSection() {
  return (
    <section
      id="form"
      className="relative bg-brand text-paper-warm scroll-mt-20 overflow-hidden"
    >
      {/* Glow dorado difuso decorativo */}
      <div
        className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full opacity-25 blur-[140px] pointer-events-none"
        style={{ background: "var(--color-brand-yellow)" }}
      />
      {/* Grid de puntos sutil */}
      <div className="absolute inset-0 bg-dot-grid opacity-50 pointer-events-none" />

      {/* Isotipo de Cauce (logo original) — elemento gráfico de fondo, recortado a la derecha */}
      <Image
        ...
        width={674}
        height={319}
        className="hidden lg:block absolute right-[-6%] top-1/2 -translate-y-1/2 w-[48%] max-w-[720px] h-auto opacity-[0.12] pointer-events-none select-none"
      />

      <div className="container-page relative py-20 md:py-28">
        {/* Header — meta-bar consistente */}
        <div className="flex flex-wrap items-center justify-between gap-4 pb-6 mb-12 border-b border-white/15">
          <div className="flex items-center gap-3">
            <span className="relative flex items-center justify-center w-3 h-3">
              ...
              <span className="relative w-1.5 h-1.5 rounded-full bg-brand-yellow shadow-[0_0_8px_rgba(212,165,92,0.6)]" />
            </span>
        ...
        {/* Layout principal — una columna, el ∞ vive en el fondo */}
        <div className="relative max-w-2xl">
          <aside>
            <h2
              className="text-[36px] sm:text-[44px] lg:text-[60px] font-display leading-[1.05] tracking-[-0.03em] text-white mb-5"
              style={{ fontWeight: 500 }}
            >
              Conocé al bot
              <br />
              <span className="text-white/55">charlando con él.</span>
            </h2>

            <p className="text-[16px] md:text-[18px] text-white/75 leading-[1.65] mb-10 max-w-xl">
            ...
            {/* Lo que te llevás */}
            <div className="mb-10">
              <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-white/50 mb-4 block">
                ▸ Lo que te llevás
              </span>
              <ul className="space-y-3">
                {DELIVERABLES.map((d, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-3 text-[15px] md:text-[16px] text-white/85 leading-[1.5]"
                  >
                    <span
                      aria-hidden
